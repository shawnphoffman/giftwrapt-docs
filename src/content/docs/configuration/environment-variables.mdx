...
# Dev-only. Skips the boot-time HeadBucket connectivity check so fake
# STORAGE_* values satisfy the configured-ness gate without reaching a
# real bucket. Leave unset anywhere that uploads real images.
STORAGE_SKIP_BOOT_CHECK=
```
...
Two opt-in families: Sentry-compatible error reporting and a Prometheus metrics endpoint. See [Observability](/configuration/observability/) for the full operator picture.

```bash
SENTRY_DSN=
SENTRY_ENVIRONMENT=production      # defaults to NODE_ENV
SENTRY_RELEASE=
SENTRY_TRACES_SAMPLE_RATE=0.1      # 0 to 1
METRICS_TOKEN=<16+ chars>
```

| Variable | What it does |
|---|---|
| `SENTRY_DSN` | Where error events go. Points at whatever you chose - your Sentry org, a self-hosted GlitchTip, your own proxy. Unset means no error reporting. |
| `SENTRY_ENVIRONMENT` | Environment tag on each event. Falls back to `NODE_ENV`. |
| `SENTRY_RELEASE` | Release tag on each event. Set it from your build's version or commit SHA. |
| `SENTRY_TRACES_SAMPLE_RATE` | Performance-trace sample rate, `0` to `1`. Unset means the SDK default. |
| `METRICS_TOKEN` | Bearer token that gates `/api/metrics` scraping. Minimum 16 characters. A real secret, unlike the DSN. |

<Aside type="caution" title="Both Gates Have to Be On">
Each family needs **both** the env var and the matching admin toggle (`enableSentry` / `enableMetrics`, both default off, under **Admin > General > Observability**). Setting `SENTRY_DSN` alone emits nothing; flipping the toggle without the env var emits nothing either. The gate is evaluated per request, so turning the admin toggle off kills emission mid-deploy without a restart.
</Aside>

`SENTRY_DSN` lives in the environment rather than `app_settings` because it's public by design - the Sentry SDK ships it in client HTML. `METRICS_TOKEN` is a genuine secret; treat it like `CRON_SECRET`.
...
# STORAGE_MAX_UPLOAD_MB=8
# STORAGE_SKIP_BOOT_CHECK=true    # dev only - skips the boot HeadBucket check
...
# ===================================================================
# Observability (optional - each family also needs its admin toggle)
# ===================================================================

# Sentry-compatible error reporting. Emits only when SENTRY_DSN is set
# AND `enableSentry` is on in Admin > General > Observability.
# SENTRY_DSN=
# SENTRY_ENVIRONMENT=production   # defaults to NODE_ENV
# SENTRY_RELEASE=                 # version or commit SHA
# SENTRY_TRACES_SAMPLE_RATE=0.1   # 0 to 1

# Bearer token gating /api/metrics. Min 16 chars. Scraping works only
# when this is set AND `enableMetrics` is on. This one is a real secret.
# METRICS_TOKEN=
```
