...
| **Intelligence (recommendations)** | Per-user analyzers that surface stale items, duplicates, grouping suggestions. | [Intelligence](/configuration/intelligence-ai/). |
...
[Intelligence](/configuration/intelligence-ai/) supports a `intelligenceModelOverride` setting that swaps just the model ID for analyzer calls. The provider, API key, and base URL still come from the global AI config; the override only swaps the model within that provider.
