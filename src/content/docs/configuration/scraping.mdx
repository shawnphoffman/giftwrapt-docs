...
| `flaresolverr` | sequential | - | Self-hosted; resolves interstitials before returning HTML |
...
| `scrapfly` | sequential | `apiKey` | Hosted rendering service |
...
| Vercel + ScrapFly | `fetch -> scrapfly` | Hosted rendering via `asp=true`. Pay per credit. |
