...
|---|---|---|
| `/api/cron/auto-archive` | `0 14 * * *` | Archives claimed items past the birthday / Christmas / holiday reveal date. This is what triggers the recipient-facing reveal. Skips lists with an active per-list extension and reveals any whose extension has elapsed. |
| `/api/cron/birthday-emails` | `0 15 * * *` | Day-of birthday greetings, post-birthday gifter summaries, pre-event reminders, relationship reminders, and the [orphan-claim cleanup passes](#orphan-claim-cleanup). |
...
All five are **daily** in the default schedule, deliberately staggered so they don't pile up at the same minute. Times are UTC.

<Aside type="tip" title="Run the Email Jobs in Your Morning">
`auto-archive` and `birthday-emails` work out which date it is from the time zone set in **Admin > Core settings**, then send that day's emails. Their defaults (14:00 and 15:00 UTC) land in the morning across the Americas and the afternoon in Europe. East of there they run late at night, and `/admin/scheduling` shows a warning. Move them to a morning time in your zone.
</Aside>
...
- **Render**: `render.yaml` generates the value for you and attaches it to both the web service and the cron jobs.
- **Vercel**: `vercel.json` declares the schedules but not the secret. Set `CRON_SECRET` yourself in the project's environment variables.
- **Railway**: set it yourself. `railway.json` carries only build and healthcheck config, and Railway's cron is wired up through the dashboard.
...
| `cooldown` | Intelligence, manual refresh only: the user hit `intelligenceManualRefreshCooldownMinutes`. |

Intelligence also skips work *below* the run level: an analyzer whose candidate slice hashes the same as the last real generation returns before its model call and carries its existing recs forward. That's a per-analyzer economy, not a run skip, so the run still records as a success. See [Intelligence](/configuration/intelligence/#per-analyzer-skips).
...
`railway.json` carries build and healthcheck config only - no schedules. Either wire up Railway's cron through the dashboard (one cron service per endpoint), or run an external scheduler.
...
The `/api/cron/birthday-emails` tick also runs two passes for the [orphaned-claim flow](/features/claims/#orphaned-claims):
