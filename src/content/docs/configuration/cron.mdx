...
|---|---|---|
| `/api/cron/auto-archive` | `0 6 * * *` | Archives claimed items past the birthday / Christmas / holiday reveal date. This is what triggers the recipient-facing reveal. Skips lists with an active per-list extension and reveals any whose extension has elapsed. |
| `/api/cron/birthday-emails` | `0 7 * * *` | Day-of birthday greetings, post-birthday gifter summaries, pre-event reminders, relationship reminders, and the [orphan-claim cleanup passes](#orphan-claim-cleanup). |
