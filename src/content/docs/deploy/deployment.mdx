...
|---|---|
| [Hosted (Vercel + Supabase)](/deploy/hosted/) | Tried-and-true managed path. The configuration I run in production. |
| [Hosted (Railway)](/deploy/railway/) | One-click multi-service template. Web + Postgres + 5 cron services in a single deploy. |
| [Hosted (Render)](/deploy/render/) | One-click `render.yaml` blueprint. Web + managed Postgres + 5 cron jobs. |
| [Self-hosting with Docker](/deploy/self-hosting/) | Full control. One `docker compose up`, runs anywhere. |
...
This lands the entire stack (app + DB + storage) without any post-deploy paste step. Full self-host walkthrough: [Self-hosting](/deploy/self-hosting/).
...
- **Vercel**: schedules ship in `vercel.json`. See [Hosted (Vercel + Supabase)](/deploy/hosted/).
- **Railway**: schedules ship as separate cron services in the template. See [Hosted (Railway)](/deploy/railway/#cron-services).
- **Render**: schedules ship as Cron Jobs in `render.yaml`. See [Hosted (Render)](/deploy/render/#cron-jobs).
- **Self-hosted Docker Compose**: a busybox crond sidecar runs alongside the app. See [Self-hosting → Scheduled jobs](/deploy/self-hosting/#addon-scheduled-jobs-cron).
