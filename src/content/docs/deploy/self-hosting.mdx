...
Self-hosting GiftWrapt is meant to be boring: one published image, one compose file, one `.env`. The base stack is app + Postgres. Everything else - storage, cron, email, MCP - is an addon you opt into when you need it.
...
`:latest` follows the main branch. Once you're past "does it work," set `APP_IMAGE=ghcr.io/shawnphoffman/giftwrapt:vX.Y.Z` in `.env` so you control when you upgrade.
</Aside>
...
That's it. You're running. If you need image uploads, scheduled jobs, email, or the MCP sidecar, keep reading.
...
The bundled compose files ship with a **cron sidecar** by default - a tiny alpine container running busybox `crond` that curls each endpoint on a daily UTC schedule. No setup beyond `CRON_SECRET`. The sidecar fatals at startup if `CRON_SECRET` is unset, so the stack boot-loops until you set it.
...
The bundled compose ships an optional **Model Context Protocol** sidecar so you can talk to your wishlist from Claude Desktop / Claude Code. It's a thin shim over the mobile API.
...
For reference, here's the complete bundled compose with all addons wired up. The canonical version lives at [`docker/compose.selfhost-garage.yaml`](https://github.com/shawnphoffman/giftwrapt/blob/main/docker/compose.selfhost-garage.yaml).
...
The RustFS variant lives at [`docker/compose.selfhost-rustfs.yaml`](https://github.com/shawnphoffman/giftwrapt/blob/main/docker/compose.selfhost-rustfs.yaml).
