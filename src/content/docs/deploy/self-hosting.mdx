...
GiftWrapt's optional [URL scraping AI extractor](/features/scraping/), title cleaner, and [Suggestions](/features/suggestions/) all share one LLM provider config. Configure it from **Admin > AI** in the UI (recommended), or pin it via env vars:
...
- `init-garage.mjs` / `init-rustfs.mjs` - bootstrap the storage bucket and keys (the entrypoint runs these for you when `INIT_GARAGE` / `INIT_RUSTFS` is set)
- `intelligence-run-once.mjs` - generate suggestions once, for testing
- `intelligence-worker.mjs` - long-running suggestions worker, for running it as its own service
...
Some releases ask for a one-time action beyond pull-and-restart, such as rotating a secret. Check [Upgrading](/reference/upgrading/) before you pull.
