...
GiftWrapt also ships config files for **Railway** ([`railway.json`](https://github.com/shawnphoffman/giftwrapt/blob/main/railway.json)) and **Render** ([`render.yaml`](https://github.com/shawnphoffman/giftwrapt/blob/main/render.yaml)). Both work - I just don't run them in production myself, so I don't document them as the recommended path.

If you're set on one of those, the config files are the source of truth; treat them as templates and adapt to your needs.

## One-click deploys (WIP)

One-click "deploy to Vercel" / "deploy to Railway" / "deploy to Render" buttons are a work in progress. The Vercel + Supabase Marketplace integration mostly works but the storage paste step still has to happen by hand, and the Railway button currently lands the app only (no database or storage wired up). Rather than ship something half-broken, I'm holding off on the buttons until the flow is genuinely one click for at least one target.

For now, the seven-step Vercel + Supabase walkthrough above is the recommended path.
