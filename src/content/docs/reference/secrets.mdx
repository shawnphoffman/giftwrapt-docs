...
<Aside type="danger" title="Rotate When Upgrading Past 0.45.0">
Releases up to and including `0.45.0` returned the decrypted OIDC client secret in the unauthenticated app-settings response that every page load prefetches. If you ran any of those versions with OIDC configured, treat the secret as public and rotate it: generate a new one at your IdP, paste it into **Admin → Auth**, and restart. Deployments that never configured OIDC are unaffected.
</Aside>
...
Keep a running log of rotations in a private location. Useful when investigating later: did we already rotate this when so-and-so was offboarded?
